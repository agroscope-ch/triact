...
> Simmler M., Brouwers S., 2023. *triact package for R: Analyzing the lying behavior of cows from accelerometer data*, under review
...
\
Inspecting one of the files reveals a file header and the semicolon-separated data starting after the line with `"*Data`". This is an example of what files imported by *triact* might look like. However, *triact* can handle any kind of delimiter-separated text files, with or without an arbitrary file header (which are ignored during import).\
...
+ With `timeFwdUpRight_cols = c(1, -2, 3, -4)` we map the columns as found in the files to the *time*, and the *forward*, *up*, and *right* accelerations as understood by *triact*. Fig. 1a shows the accelerometer used to collect the example data with the axis directions as defined by the manufacturer (XYZ). Fig. 1b shows the directions as used in *triact*. To collect the example data, the accelerometer was mounted to the outside of the left hind leg with the Y axis pointing in *Up* direction, X in opposite direction of *forward* (directed backwards), and Z in opposite direction of *right* (directed left). In `timeFwdUpRight_cols = c(1, -2, 3, -4)`, the first number indicates in which column in the file the timestamp (data-time) is located, here in the first. The second number indicates in which column in the file maps to *forward* acceleration, here the second column, but with negative mathematical sign as X was pointing in the opposite direction of *forward* (hence -2). The third number indicates which column in the file maps to the *up* axis, here the third (the Y data). The last number indicates which column in the file maps to the *right* acceleration, here the forth, but with opposite mathematical sign as Z was pointing in the opposite direction of *right* (hence -4).
...
If you are unsure of the directions of your accelerator's XYZ data, you can determine them by experimentation. If the axis in question is pointing up, it should measure +1 *g* at rest. Note that accelerators measure *proper acceleration* which is relative to free fall, so the gravitational components points skywards (reaction force of the ground).     
...
You can import multiple files from the same cows (unique ID), but the data should follow each other without gaps in time. If you have gaps, you should use unique ids for the time series without gaps, e.g. you may need to use the cow identifier in combination with the date as unique ID. 
...
Calling add_... methods triggers analyses of lying behavior and the calculation of proxies for the level of physical activity. The results of the analyses are obtained for each time point of your acceleration data and added in new columns to the tabular data in the `Triact` object.
...
The `$add_lying()` method comes with many parameters that allow you to tweak the underlying algorithm. With the default parameters, you can expect good results if your accelerator sampling frequency is \≥1 Hz. See `?Triact` and Simmler & Brouwers (2023) for explanations.
...
The `$add_activity()` method performs the calculation of proxies for the level of physical activity of the cow(s). By default, the L2 norm of the vector of the dynamic body acceleration (DBA) is calculated and 'adjusted' to  a value of zero during lying bouts. The results are (silently) added to the data in the `Triact` object as numeric column names *AdjL2DBA*.
...
You can request all available proxies for physical activity, namely L1 and L2 norms of DBA and Jerk, by specifying the parameters `dynamic_measure` and `norm` accordingly. This adds the columns `AdjL2DBA`, `AdjL1DBA`, `AdjL1Jerk`, and `AdjL2Jerk`. For the first timestamp of each id, the jerk-based proxies are NA (not available) because they are based on differences between successive acceleration values. See Simmler & Brouwers (2023) for a discussion of these proxies. 

```{r}
my_triact$add_activity(dynamic_measure = c("dba", "jerk"), 
                       norm = c("L1", "L2"))
```
...
Calculating DBA-based proxies involves a filtering step that can be tuned with several parameters (see `?Triact`). With the default parameters, you can expect good results if your accelerator sampling frequency is \≥1 Hz.
